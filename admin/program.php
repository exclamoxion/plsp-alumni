...
<?php
    if (isset($_POST['save'])) {
        ...
        $query = mysqli_query($connection, "SELECT * from program where description = '$description'  ");
        if (mysqli_num_rows($query) > 0) {
            echo ('<script>alert("Data Existed");</script>');
        } else {
            mysqli_query($connection, "insert into program (title,description) values ('$title','$description')") or die($connection->error);
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php
                    if (isset($_GET['id'])) {
                        $course = mysqli_query($conn, "SELECT * FROM program where program_id = {$_GET['id']}");
                    ...
                    <form class="form-horizontal" method="POST">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="title" id="inputPassword" class="form-control" value="<?php echo isset($title) ? $title : '' ?>" placeholder="Title" required>
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" for="inputPassword">Description</label>
                            <div class="controls">
                                <input type="text" name="description" id="inputPassword" class="form-control" value="<?php echo isset($description) ? $description : '' ?>" placeholder="Description" required>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="table-responsive pb-5">
                                        <br>
                                        <button type="button" class="btn btn-primary mb-1 bx bxs-user-plus" data-toggle="modal" data-target="#exampleModal"> Add Program</button>
                                        <br>
                                        <br>
                                        <table
                                            ...
                                            <tbody>
                                                <?php
                                                $query = mysqli_query($conn, "select * FROM program ") or die($conn->error);
                                                while ($row = mysqli_fetch_array($query)) {
                                                    $id  = $row['program_id'];
                                                ?>
                                                <tr>

                                                    <td class="text-uppercase"><?php echo $row['title']; ?></td>
